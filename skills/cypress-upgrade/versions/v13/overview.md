...
This path's breaking changes are configuration and behavior changes (see [./breaking-changes.md](./breaking-changes.md)); it defines **no install-blocker or dependency-version flags**, so the precheck has nothing to flag here and the install can proceed. For full environment requirements (Node.js, OS), consult the [changelog](https://on.cypress.io/changelog#13-0-0).
